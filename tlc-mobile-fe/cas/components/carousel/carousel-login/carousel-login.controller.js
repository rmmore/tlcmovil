...
(function(angular) {	
	...
	function LoginCarouselController(
		...
		function initStates() {
			...
			var localAppValue = $storageService.getAppStorage();//Obtiene de localStorage

			//Si se accede desde app
			if(vm.isApp || localAppValue != null){
				$storageService.setAppStorage(vm.isApp);//Almacena en localStorage
				vm.isApp = true;
			}
		}
		...
		function next() {
			...
			if($rootScope.flagFrequentCardEnable && !vm.card.cardName){//RPA
				$modalService.open({
					title: $coreConstants.ERROR,
					description: $coreConstants.MSG_FREQUENT_CARD_INCOMPLETE,
					showCancel: false
				});
		...
		function login() {
			...
			if(vm.password.length !== $coreConstants.PASSWORD_LENGTH) {
				$modalService.open({
					title: $coreConstants.ERROR,
					description: $coreConstants.MSG_PARCIAL_PASSWORD,
					showCancel: false
				});
				return;
			}
